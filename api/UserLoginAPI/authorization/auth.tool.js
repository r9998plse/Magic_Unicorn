...
AuthTool.isLoginValid = async (req, res, next) => {
  ...
  if (req.body.email) user = await UserModel.getByEmail(req.body.email)
...
AuthTool.isRefreshValid = async (req, res, next) => {
  if (!req.body || !req.body.refresh_token) return res.status(400).send()

  if (!req.headers['authorization']) return res.status(401).send()

  if (typeof req.body.refresh_token !== 'string') return res.status(400).send()
...
AuthTool.isSameUserOr = (required_permission) => {
  return (req, res, next) => {
    ...
    let same_user =
      req.jwt.userId === userId ||
      req.jwt.username.toLowerCase() === userId.toLowerCase()
    if (userId && same_user) {
      return next()
    } else {
      if (user_permission_level >= required_permission) {
